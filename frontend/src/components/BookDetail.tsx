...
export function BookDetail({ book, courses, onSelectCourse }: BookDetailProps) {
  const stats = useMemo(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        {stats.bookCourses.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center text-muted-foreground">
              This book has no courses yet. Open a chapter to generate a course.
            </CardContent>
          </Card>
        ) : (
          stats.bookCourses.map((course) => {
            ...
            return (
              <Card key={course.id} className="cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => onSelectCourse(course)}>
